...
opts.train.expDir = './data/vgg19_coco_batch32_Rankloss_2:1:0.1_margin1_both_drop0.5_hard_256';
...
opts.train.learningRate = [0.1*ones(1,15),0.01*ones(1,5)] ;
...
%net.vars(net.getVarIndex('x0')).value = [];
...
net.vars(net.getVarIndex('prediction_img')).value = [];
f_txt = getFeature2(net,txtinput_rand,[],'data2','fc2bn_n');
size4 = size(f_txt,4);
...
%reset(net);
%--
inputs = {'x0',gpuArray(oim),'data2',gpuArray(txtinput),'label_img',label_img,'label_txt',label_txt};
